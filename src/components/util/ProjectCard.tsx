...
const ProjectCard = ({data, className}: {data: any, className?: string}) => {
    return (
        <Link to={URL.FUND.replace(':uid', data.uid)} className={"flex flex-col text-sm text-gray-500 bg-white shadow-sm transition-all duration-200 hover:shadow-xl hover:-translate-y-3 rounded-md " + className}>
