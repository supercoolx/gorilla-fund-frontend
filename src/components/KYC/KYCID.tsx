    ...
    return (
        <>
            ...
            <div className="grid w-full grid-cols-3 gap-3 pt-6 text-gray-500">
                <div onClick={changeType(1)} className={`flex items-center gap-3 border-[1px] px-2 py-4 cursor-pointer ${identifyType === 1 && "border-teal-700"}`}>
                ...
                <div onClick={changeType(2)} className={`flex items-center gap-3 border-[1px] px-2 py-4 cursor-pointer ${identifyType === 2 && "border-teal-700"}`}>
                    <img src={identifyType === 2 ? iconID : iconIDGray} alt="" />
                    <div className={`font-semibold ${identifyType === 2 && "text-teal-700"}`}>National ID</div>
                </div>
                <div onClick={changeType(3)} className={`flex items-center gap-3 border-[1px] px-2 py-4 cursor-pointer ${identifyType === 3 && "border-teal-700"}`}>
            ...
            <div className="flex flex-col w-full pt-6">
                ...
                <input type="text" value={identifyNumber} onChange={changeNumber} className="w-full py-2 px-3 focus:outline-none border-[1px] border-slate-200" placeholder="Identity Number" />
            </div>
            <div className="flex flex-col w-full pt-6">
                <div className="pb-1 font-bold">Date of Expiry*</div>
                <input type="date" value={identifyExpire} onChange={changeExpire} className="w-full py-2 px-3 focus:outline-none border-[1px] border-slate-200" placeholder="Identity Number" />
            </div>
            <div className="flex flex-col w-full pt-6">
